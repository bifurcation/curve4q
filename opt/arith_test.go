...
func wadd(x, y, c uint64) (z1, z0 uint64) {
	...
	*/
	...
	lo := (x & 0xffffffff) + (y & 0xffffffff) + c
	carry := (lo >> 32)
	lo -= (carry << 32)
	hi := (x >> 32) + (y >> 32) + carry
...
func fpmul(x, y fpelt) (z fpelt) {
	...
	// This is wrong, but should give approx right opcounts
	z0 := x0y0 + x1y3 + x2y2 + x3y1
	z1 := x0y1 + x1y0 + x2y3 + x3y2
	z2 := x0y2 + x1y1 + x2y0 + x3y3
	z3 := x0y3 + x1y2 + x2y1 + x3y0

	// Also wrong, but should give approx right opcounts
	carry := z0 >> 32
	z1 += carry
	z0 -= (carry << 32)
